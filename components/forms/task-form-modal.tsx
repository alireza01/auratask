...
import { Loader2, CalendarIcon, Sparkles, X } from "lucide-react"
...
export function TaskFormModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (settings?.gemini_api_key && !editingTask) {
    ...
    <Dialog open={isTaskFormOpen} onOpenChange={closeTaskForm}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>{editingTask ? "ویرایش وظیفه موجود" : "ایجاد وظیفه جدید"}</DialogDescription>
        </DialogHeader>
